...
export default function Results() {
  const [students10, setStudents10] = useState<Result[]>([])
  const [students12, setStudents12] = useState<Result[]>([])

  useEffect(() => {
    fetch(API)
      .then(res => res.json())
      .then((data: Result[]) => {
        setStudents10(data.filter(r => r.className.includes("10")).slice(0, 5))
        setStudents12(data.filter(r => r.className.includes("12")).slice(0, 5))
      })
